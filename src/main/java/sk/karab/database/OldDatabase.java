...
public abstract class OldDatabase {
    ...
    public int tokens = 0;
    public OldDatabase(FPinata instance){
        plugin = instance;
    }
    ...
    public Integer getTokens(String string) {
        ...
        try {
            ...
            plugin.getLogger().log(Level.SEVERE, OldErrors.sqlConnectionExecute(), ex);
        } finally {
            try {
                if (ps != null)
                    ps.close();
                ...
                plugin.getLogger().log(Level.SEVERE, OldErrors.sqlConnectionClose(), ex);
            }
        }
        return 0;
    }
